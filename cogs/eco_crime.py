...
class Crime(commands.Cog):
    ...
    @app_commands.command(name="crime", description="Cometa um delito contra o sistema. Pode dar bom ou tu vai pro Serasa.")
    @app_commands.choices(tipo=[
        app_commands.Choice(name="💳 Fraudar o INSS (Risco Baixo)", value="inss"),
        app_commands.Choice(name="🚗 Desmanche de Marea (Risco Médio)", value="desmanche"),
        app_commands.Choice(name="🏦 Assaltar o Banco Central (Risco Extremo)", value="banco")
    ])
    async def crime(self, interaction: discord.Interaction, tipo: app_commands.Choice[str]):
        ...
        if user_id in self.cooldown_crime:
            ...
            if tempo_passado < 7200:
                ...
                return await interaction.response.send_message(f"⏳ A polícia tá na tua cola, xupeta! Fica escondido por **{horas}h e {minutos}m** antes de tentar outra merda.", ephemeral=True)
        ...
        if not res:
            conn.close()
            return await interaction.response.send_message("🌀 O vácuo não te reconhece. Mande uma mensagem no chat primeiro.", ephemeral=True)
        ...
        if saldo_atual < -1000:
            conn.close()
            return await interaction.response.send_message("❌ Teu nome já tá sujo demais no Serasa do Vácuo (Saldo menor que -1000). Vai trabalhar, liso, não tem nem como financiar o crime!", ephemeral=True)
        ...
        if sucesso:
            ...
            c.execute("UPDATE usuarios SET moedas = moedas + ?, xp = xp + ? WHERE user_id = ?", (lucro, xp_ganho, str(user_id)))
            embed = discord.Embed(title=titulo, description=msg_win, color=0x00FF00)
            embed.set_footer(text=f"Aham q lindo me paga um boquete dps de lucrar. (+{xp_ganho} XP)")
        else:
            c.execute("UPDATE usuarios SET moedas = moedas - ? WHERE user_id = ?", (multa, str(user_id)))
