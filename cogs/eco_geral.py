...
class DailyView(discord.ui.View):
    ...
    async def abrir_caixa(self, interaction: discord.Interaction, indice: int, num_caixa: int):
        if interaction.user.id != self.autor.id:
            await interaction.response.send_message("❌ Sai daqui xupeta, essa caixa não é tua! Ai dento.", ephemeral=True)
            return
        ...
        c = conn.cursor()
        c.execute("UPDATE usuarios SET moedas = moedas + ? WHERE user_id = ?", (premio_ganho, str(self.autor.id)))
        conn.commit()
        conn.close()
...
class TrabalharView(discord.ui.View):
    ...
    async def processar_trabalho(self, interaction: discord.Interaction, escolha: str):
        if interaction.user.id != self.autor.id:
            await interaction.response.send_message("❌ Vai caçar tua própria CLT, mísera!", ephemeral=True)
            return
        ...
        if escolha == self.palavra_certa:
            conn = sqlite3.connect(DB_PATH)
            c = conn.cursor()
            c.execute("UPDATE usuarios SET moedas = moedas + ? WHERE user_id = ?", (self.recompensa, str(self.autor.id)))
            conn.commit()
            conn.close()
...
class Economia(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        conn = sqlite3.connect(DB_PATH)
        cursor = conn.cursor()
        cursor.execute("SELECT xp, nivel FROM usuarios WHERE user_id = ?", (str(user_id),))
        res = cursor.fetchone()

        if res:
            xp_atual, nivel_atual = res
            novo_xp = xp_atual + xp_ganho
            proximo_nivel = (nivel_atual + 1) * 500

            if novo_xp >= proximo_nivel:
                novo_nivel = nivel_atual + 1
                cursor.execute("UPDATE usuarios SET xp = ?, nivel = ?, moedas = moedas + ? WHERE user_id = ?", 
                               (novo_xp - proximo_nivel, novo_nivel, moedas_ganhas, str(user_id)))
                await message.channel.send(f"📈 **MILAGRE!** {message.author.mention} sofreu, mas upou pro **Nível {novo_nivel}**! Menos um liso no server.")
            else:
                cursor.execute("UPDATE usuarios SET xp = ?, moedas = moedas + ? WHERE user_id = ?", 
                               (novo_xp, moedas_ganhas, str(user_id)))
            conn.commit()
        conn.close()

    # --- 2. COMANDO DE TRABALHO ---
    @app_commands.command(name="trabalhar", description="Presta um serviço de hacker e ganha umas moedas (Cooldown de 4h).")
    async def trabalhar(self, interaction: discord.Interaction):
        ...
        if user_id in self.cooldown_trabalho:
            ...
            if tempo_passado < 14400:
                ...
                await interaction.response.send_message(f"⏳ Ai dento! A carga horária acabou. Volta daqui a **{horas_restantes}h e {minutos_restantes}m**, CLT!", ephemeral=True)
                return
        ...
        palavras = ["VACUO", "XUPETA", "MISERA", "SHITPOST", "MACACO", "AGIOTA", "CALVO", "DEBOCHE", "HACKER", "SISTEMA"]
    ...
    @app_commands.command(name="daily", description="Pega tua esmola diária pra não passar fome.")
    async def daily(self, interaction: discord.Interaction):
        ...
        if user_id in self.cooldown_daily:
            ...
            if tempo_restante > 0:
                ...
                await interaction.response.send_message(f"⏳ Ai dento! Quer farmar infinito, mísera? A esmola só libera daqui a **{horas}h e {minutos}m**. Vai caçar o que fazer!", ephemeral=True)
                return
