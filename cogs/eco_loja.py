    ...
    @discord.ui.select(
        ...
        options=[
            discord.SelectOption(
                label="🛡️ VPN do Agiota (Escudo)",
                description="Ninguém consegue te roubar por 24h",
                value="buy_escudo",
                emoji="🛡️",
            ),
            discord.SelectOption(
                label="🧠 Curso de Coach Financeiro",
                description="[EM BREVE] Aumenta o limite de XP",
                value="coming_1",
                emoji="🧠",
            ),
            discord.SelectOption(
                label="🌑 RG Falso",
                description="[EM BREVE] Fique invisível no Ranking",
                value="coming_2",
                emoji="🌑",
            ),
            discord.SelectOption(
                label="💎 VIP do Cabaré",
                description="[EM BREVE] Acesso a comandos VIP",
                value="coming_3",
                emoji="💎",
            ),
        ],
    ...
    async def shop_callback(
        ...
        if "coming" in select.values[0]:
            await interaction.response.send_message(
                "🌀 Calma aí mísera, o fornecedor ainda não entregou essa muamba no porto. Aguarde.",
                ephemeral=True,
            )
            return
        ...
        if select.values[0] == "buy_escudo":
            ...
            cursor = conn.cursor()
            cursor.execute(
                "SELECT moedas FROM usuarios WHERE user_id = ?",
            ...
            if not res or res[0] < preco:
                await interaction.response.send_message(
                    "❌ Tu é liso, xupeta! Volta quando tiver as `1.500` moedas de verdade.",
                    ephemeral=True,
                )
            else:
                expira = (
                ...
                cursor.execute(
                    "UPDATE usuarios SET moedas = moedas - ?, escudo_ate = ? WHERE user_id = ?",
                    (preco, expira, str(interaction.user.id)),
                )
                conn.commit()
                await interaction.response.send_message(
                    "🛡️ **VPN do Agiota Ativada!** Tá blindado contra roubo por 24 horas. Aham q lindo me paga um boquete dps de ligar o escudo!",
                    ephemeral=True,
                )
            conn.close()
...
class Loja(commands.Cog):
    ...
    @app_commands.command(
        name="loja", description="Abre o camelô clandestino do servidor."
    )
    async def loja(self, interaction: discord.Interaction):
        ...
        embed.set_author(
            name=f"Vendedor: {self.bot.user.name}",
