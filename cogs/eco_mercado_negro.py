...
class MercadoNegroDropdown(discord.ui.Select):
    def __init__(self):
        ...
        super().__init__(
            placeholder="Escolhe a muamba, anda logo...",
            min_values=1,
            max_values=1,
            options=opcoes,
        )
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        if escolha in ["pe_de_cabra", "suborno"]:
            return await interaction.response.send_message(
                "❌ Tá cego, xupeta? Essa porra tá sem estoque. Volta na próxima atualização do bot.",
                ephemeral=True,
            )
        ...
        if not res:
            conn.close()
            return await interaction.response.send_message(
                "🌀 Tu nem existe no sistema. Vai mandar mensagem no chat primeiro.",
                ephemeral=True,
            )
        ...
        if escolha == "rg_falso":
            ...
            if saldo_atual < preco_rg:
                conn.close()
                return await interaction.response.send_message(
                    f"💀 Tu é liso e burro. Como tu vai comprar um RG Falso de `{preco_rg}` moedas se tu só tem `{saldo_atual}`? Pede dinheiro pra tua webnamorada!",
                    ephemeral=True,
                )
            ...
            embed.set_thumbnail(
                url="https://i.imgur.com/q3LXXHh.png"
            )  # Uma imagem genérica de hacker/anon
...
class MercadoNegro(commands.Cog):
    ...
    @app_commands.command(
        name="biqueira",
        description="Acesse a loja clandestina. Itens ilegais e falsificações.",
    )
