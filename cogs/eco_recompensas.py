...
class Recompensas(commands.Cog):
    ...
    grupo = app_commands.Group(
        name="encomenda", description="Mercado de matadores de aluguel."
    )

    @grupo.command(name="colocar", description="Bote a cabeça de um xupeta a prêmio.")
    async def colocar_recompensa(
        self, interaction: discord.Interaction, alvo: discord.Member, valor: int
    ):
        if alvo.id == interaction.user.id or alvo.bot:
            return await interaction.response.send_message(
                "❌ Tu quer botar tua própria cabeça a prêmio? Deixa de ser doente.",
                ephemeral=True,
            )

        if valor < 1000:
            return await interaction.response.send_message(
                "❌ O Agiota não suja a mão por menos de 1.000 moedas.", ephemeral=True
            )
        ...
        if not res or res[0] < valor:
            conn.close()
            return await interaction.response.send_message(
                f"💀 Tu não tem `{valor}` moedas pra pagar o contrato, liso.",
                ephemeral=True,
            )
        ...
        c.execute(
            """INSERT INTO recompensas (alvo_id, contratante_id, valor) 
                     VALUES (?, ?, ?) 
                     ON CONFLICT(alvo_id) 
    ...
    @grupo.command(
        name="cacar", description="Tente abater um procurado e levar a grana do mural."
    )
    async def cacar_alvo(self, interaction: discord.Interaction, alvo: discord.Member):
        if alvo.id == interaction.user.id:
            return await interaction.response.send_message(
                "❌ Vai se matar?", ephemeral=True
            )
        ...
        if not res_recompensa:
            conn.close()
            return await interaction.response.send_message(
                "🌀 Esse cara não tem recompensa na cabeça. Tá matando de graça por quê?",
                ephemeral=True,
            )
        ...
        if sucesso:
            ...
            prejuizo = int(
                valor_premio * 0.10
            )  # Perde 10% do valor do prêmio como taxa médica
            c.execute(
                "UPDATE usuarios SET moedas = moedas - ? WHERE user_id = ?",
                (prejuizo, user_id),
            )
