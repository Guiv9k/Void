...
class RouboView(discord.ui.View):
    ...
    async def processar_roubo(self, interaction: discord.Interaction, cor_escolhida: str):
        if interaction.user.id != self.autor.id:
            await interaction.response.send_message("❌ Sai pra lá xupeta, o roubo não é teu!", ephemeral=True)
            return
        ...
        cursor.execute("SELECT moedas FROM usuarios WHERE user_id = ?", (str(self.vitima.id),))
        ...
        if cor_escolhida == self.fio_certo:
            ...
            cursor.execute("UPDATE usuarios SET moedas = moedas + ? WHERE user_id = ?", (valor_roubado, str(self.autor.id)))
            cursor.execute("UPDATE usuarios SET moedas = moedas - ? WHERE user_id = ?", (valor_roubado, str(self.vitima.id)))
            ...
            cursor.execute("UPDATE usuarios SET moedas = moedas - ? WHERE user_id = ?", (multa, str(self.autor.id)))
    ...
    @discord.ui.button(label="FIO VERMELHO", style=discord.ButtonStyle.danger, emoji="🔴")
    async def btn_vermelho(self, interaction: discord.Interaction, button: discord.ui.Button):
...
class Roubo(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
        self.cooldown_roubo = {} # Meu nobre, esse dicionário controla as 4 horas de espera
    ...
    @app_commands.command(name="roubar", description="Tente hackear o cofre de um CLT (Cooldown de 4h).")
    async def roubar(self, interaction: discord.Interaction, vitima: discord.Member):
        if vitima.id == interaction.user.id or vitima.bot:
            await interaction.response.send_message("🌀 Tá esquizofrênico? Tenta roubar alguém de verdade, mísera.", ephemeral=True)
            return
        ...
        if user_id in self.cooldown_roubo:
            ...
            if tempo_passado < 14400:
                ...
                await interaction.response.send_message(f"⏳ Ai dento! Tu já tá sendo procurado pela polícia. Esconde a cara por **{horas_restantes}h e {minutos_restantes}m** antes de roubar de novo.", ephemeral=True)
                return
        ...
        cursor.execute("SELECT moedas, escudo_ate FROM usuarios WHERE user_id = ?", (str(vitima.id),))
        dados_vitima = cursor.fetchone()

        if not saldo_autor or saldo_autor[0] < 200:
            await interaction.response.send_message("💀 Tu é liso demais! Precisa de pelo menos 200 moedas pra bancar as ferramentas de invasão.", ephemeral=True)
            conn.close()
            return
            
        if not dados_vitima or dados_vitima[0] < 50:
            await interaction.response.send_message("🌀 O cofre desse frango tá mais vazio que tua geladeira. Procura alguém mais rico.", ephemeral=True)
            conn.close()
            return
        ...
        if dados_vitima[1]:
            try:
                ...
                if datetime.datetime.now() < data_escudo:
                    await interaction.response.send_message(f"🛡️ **DEU RUIM!** O {vitima.display_name} tá usando a **VPN do Agiota**. Tuas ferramentas não conseguiram passar. Tenta dps!", ephemeral=True)
