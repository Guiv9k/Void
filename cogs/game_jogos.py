...
class DueloGameView(discord.ui.View):
    def __init__(self, p1, p2, aposta, cor_certa):
        ...
        botoes = [
            discord.ui.Button(label="VERMELHO", style=discord.ButtonStyle.danger, custom_id="vermelho"),
            discord.ui.Button(label="AZUL", style=discord.ButtonStyle.primary, custom_id="azul"),
            discord.ui.Button(label="VERDE", style=discord.ButtonStyle.success, custom_id="verde"),
            discord.ui.Button(label="CINZA", style=discord.ButtonStyle.secondary, custom_id="cinza")
        ]
    ...
    def criar_callback(self, cor_clicada):
        async def callback(interaction: discord.Interaction):
            ...
            if interaction.user not in [self.p1, self.p2]:
                await interaction.response.send_message("❌ Sai do meio da bala bala perdida, mísera!", ephemeral=True)
                return
            if self.clicado: return
            self.clicado = True
            ...
            c = conn.cursor()
            c.execute("UPDATE usuarios SET moedas = moedas + ? WHERE user_id = ?", (self.aposta, str(vencedor.id)))
            c.execute("UPDATE usuarios SET moedas = moedas - ? WHERE user_id = ?", (self.aposta, str(perdedor.id)))
            conn.commit()
            conn.close()
...
class Jogos(commands.Cog):
    ...
    @app_commands.command(name="tiroteio", description="Desafie um xupeta. Teste seus reflexos com cores.")
    async def tiroteio(self, interaction: discord.Interaction, oponente: discord.Member, aposta: int):
        if oponente.id == interaction.user.id or oponente.bot:
            await interaction.response.send_message("🌀 Tá esquizofrênico, mísera? Escolhe alguém de verdade.", ephemeral=True)
            return
        if aposta <= 0:
            await interaction.response.send_message("💸 Aposta mixuruca não vale. Coloca dinheiro de verdade.", ephemeral=True)
            return
        ...
        if saldo_autor < aposta:
            return await interaction.response.send_message(f"❌ Tu é liso, parceiro! Não tem `{aposta:,}` moedas.", ephemeral=True)
        if saldo_alvo < aposta:
            return await interaction.response.send_message(f"❌ O {oponente.display_name} é um frango e não tem `{aposta:,}` moedas pra cobrir a aposta.", ephemeral=True)
        ...
        class DueloAcceptView(discord.ui.View):
            ...
            @discord.ui.button(label="Cair pro Pau", style=discord.ButtonStyle.danger, emoji="⚔️")
            async def aceitar(self, inter: discord.Interaction, button: discord.ui.Button):
                if inter.user.id != oponente.id:
                    await inter.response.send_message("❌ Só o desafiado pode aceitar, intrometido.", ephemeral=True)
                    return
                self.aceito = True
                self.stop()
                
                embed_prep = discord.Embed(title="⚠️ PREPARAR...", description="Dedos no gatilho. Leiam a cor que eu vou gritar na próxima mensagem!", color=0xFF0000)
                await inter.response.edit_message(embed=embed_prep, view=None)

        view = DueloAcceptView()
        await interaction.response.send_message(content=oponente.mention, embed=embed_convite, view=view)
        await view.wait()
        ...
        if view.aceito:
            ...
            msg = await interaction.edit_original_response(embed=embed_game, view=game_view)
            
            await game_view.wait()
            if not game_view.clicado:
                embed_empate = discord.Embed(title="🏳️ EMPATE DE XUPETAS", description="Os dois dormiram no ponto. O tiroteio foi cancelado porque faltou mira.", color=0x2b2d31)
                await interaction.edit_original_response(embed=embed_empate, view=None)
        else:
            await interaction.edit_original_response(content=f"🐔 **{oponente.display_name}** arregou, meteu o pé e fugiu do x1.", embed=None, view=None)

    # --- JOGO: ROLETA EXTREMA ---
    @app_commands.command(name="roleta_extrema", description="[TUDO OU NADA] Aposta todo o teu dinheiro. 50% de chance de dobrar, 50% de perder tudo.")
    async def roleta_extrema(self, interaction: discord.Interaction):
        ...
        if saldo_atual <= 0:
            conn.close()
            return await interaction.response.send_message("❌ Ai dento! Tu já tem 0 moedas, vai apostar o quê? O caneco? Vai trampar!", ephemeral=True)

        # bros, essa roleta é 50/50. Metade do tambor tem bala.
        sucesso = random.choice([True, False])

        if sucesso:
            c.execute("UPDATE usuarios SET moedas = moedas * 2 WHERE user_id = ?", (str(interaction.user.id),))
            ...
            c.execute("UPDATE usuarios SET moedas = 0 WHERE user_id = ?", (str(interaction.user.id),))
            embed = discord.Embed(
                title="💀 F DE XUPETA",
