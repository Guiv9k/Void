...
class RoletaView(discord.ui.View):
    ...
    async def atualizar_banco(self, valor, ganhar=True):
        ...
        c = conn.cursor()
        if ganhar:
            c.execute("UPDATE usuarios SET moedas = moedas + ? WHERE user_id = ?", (valor, str(self.autor.id)))
        else:
            c.execute("UPDATE usuarios SET moedas = moedas - ? WHERE user_id = ?", (valor, str(self.autor.id)))
        conn.commit()
        conn.close()

    @discord.ui.button(label="Puxar Gatilho", style=discord.ButtonStyle.danger, emoji="🔫")
    async def btn_atirar(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.autor.id:
            await interaction.response.send_message("❌ Sai do meio, xupeta! A arma não é tua.", ephemeral=True)
            return
    ...
    @discord.ui.button(label="Arregar e Levar a Grana", style=discord.ButtonStyle.success, emoji="🏃‍♂️")
    async def btn_parar(self, interaction: discord.Interaction, button: discord.ui.Button):
        if interaction.user.id != self.autor.id:
            await interaction.response.send_message("❌ Vai arrumar sua própria grana, intrometido.", ephemeral=True)
            return
            
        if self.cliques_sobrevividos == 0:
            await interaction.response.send_message("❌ Tem que puxar o gatilho pelo menos uma vez, frango covarde! Ai dento.", ephemeral=True)
            return
        ...
        embed.set_thumbnail(url=self.bot.user.display_avatar.url)
        embed.set_footer(text="Aham q lindo me paga um boquete dps de lucrar tudo isso.")
        await interaction.response.edit_message(embed=embed, view=self)
        self.stop()
...
class Roleta(commands.Cog):
    ...
    @app_commands.command(name="roleta", description="Aposte sua vida e suas moedas. Risco extremo.")
    async def roleta(self, interaction: discord.Interaction, aposta: int):
        if aposta <= 0:
            await interaction.response.send_message("❌ Aposta de centavo não rola, xupeta. Coloca um valor de verdade.", ephemeral=True)
            return
        ...
        if not res or res[0] < aposta:
            await interaction.response.send_message("💀 Tu é liso, parceiro! Não tem moedas suficientes pra essa loucura.", ephemeral=True)
            return
