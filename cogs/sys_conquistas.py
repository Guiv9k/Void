...
class Conquistas(commands.Cog):
    ...
    @app_commands.command(
        name="conquistas",
        description="Veja a sua estante de troféus e sincronize os novos.",
    )
    async def conquistas(
        ...
        if alvo.id == interaction.user.id:
            c.execute(
                "SELECT moedas, nivel FROM usuarios WHERE user_id = ?", (str(alvo.id),)
            )
            stats = c.fetchone()
            if stats:
                moedas, nivel = stats
                ...
                if moedas >= 10000:
                    novos_trofeus.append("👑 Capitalista Safado (10k Moedas)")
                if moedas >= 100000:
                    novos_trofeus.append("🏦 Dono da Porra Toda (100k Moedas)")
                if moedas <= 10:
                    novos_trofeus.append("🗑️ Liso e Frango (Quase 0 Moedas)")
                if nivel >= 5:
                    novos_trofeus.append("🧬 Mutante de Respeito (Nível 5)")
                if nivel >= 15:
                    novos_trofeus.append("🌌 Entidade do Shape Cósmico (Nível 15)")
                ...
                for trofeu in novos_trofeus:
                    try:
                        c.execute(
                            "INSERT INTO conquistas (user_id, conquista) VALUES (?, ?)",
                            (str(alvo.id), trofeu),
                        )
        ...
        embed = discord.Embed(
            title=f"🏆 MURAL DE CONQUISTAS: {alvo.display_name.upper()}", color=0xFFD700
        )
        ...
        embed.set_footer(
            text="O Vácuo tá de olho no teu progresso. Vai treinar, meu bem.",
