...
class Eventos(commands.Cog):
    ...
    async def on_member_join(self, member):
        ...
        if not canal:
            canal = discord.utils.find(
                lambda c: c.name in ["geral", "chat", "chat-geral"]
        ...
        if canal:
            ...
            c = conn.cursor()
            c.execute(
                "INSERT OR IGNORE INTO usuarios (user_id, moedas, nivel, xp) VALUES (?, ?, ?, ?)",
                (str(member.id), 0, 1, 0),
            )
            conn.commit()
            conn.close()
            ...
            embed.set_footer(
                text="Bem-vindo ao hospício.", icon_url=self.bot.user.display_avatar.url
            )
    ...
    async def on_member_remove(self, member):
        ...
        if canal:
            ...
            embed.set_thumbnail(url=member.display_avatar.url)
            embed.set_footer(
                text="Menos um CLT por aqui.", icon_url=self.bot.user.display_avatar.url
            )
            await canal.send(embed=embed)
