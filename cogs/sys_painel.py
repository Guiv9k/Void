...
class Painel(commands.Cog):
    ...
    async def gerar_embed_xp(self, guilda):
        ...
        c.execute("SELECT user_id, xp, nivel FROM usuarios ORDER BY nivel DESC, xp DESC LIMIT 10")
        top_10 = c.fetchall()
        conn.close()

        embed = discord.Embed(title="🏆 RANKING DOS DESOCUPADOS (XP)", color=0x8A2BE2, timestamp=datetime.datetime.now())
        texto = ""
        for i, user_data in enumerate(top_10):
            ...
            rank = i + 1
            icone = "🥇" if rank == 1 else "🥈" if rank == 2 else "🥉" if rank == 3 else "💀"
            texto += f"> **{icone} #{rank}** | 👤 **{nome}**\n> 🧬 Nível: `{nivel}` | ✨ XP: `{xp:,}`\n> ───────────────\n"

        embed.description = texto if texto else "Ninguém upou nada ainda. Bando de CLT ocupado."
        embed.set_footer(text="Atualizado a cada 10 min pra expor viciado", icon_url=self.bot.user.display_avatar.url)
        return embed

    async def gerar_embed_voz(self, guilda):
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT user_id, tempo_voz FROM usuarios WHERE tempo_voz > 0 ORDER BY tempo_voz DESC LIMIT 10")
        top_10 = c.fetchall()
        conn.close()

        embed = discord.Embed(title="🎙️ RANKING DOS ZÉ RADINHO (CALL)", color=0x00FFFF, timestamp=datetime.datetime.now())
        texto = ""
        for i, user_data in enumerate(top_10):
            ...
            nome = membro.display_name if membro else f"Fugitivo ({uid})"
            rank = i + 1
            icone = "🎧" if rank == 1 else "🥈" if rank == 2 else "🥉" if rank == 3 else "🎙️"
            texto += f"> **{icone} #{rank}** | 👤 **{nome}**\n> ⏳ Tempo de Fofoca: `{self.formatar_tempo(tempo)}`\n> ───────────────\n"

        embed.description = texto if texto else "Ninguém entrou em call. Bando de antissocial do caralho."
        embed.set_footer(text="Atualizado a cada 10 min", icon_url=self.bot.user.display_avatar.url)
        return embed

    # --- COMANDO PARA CRIAR O PAINEL (Só Admins) ---
    @app_commands.command(name="criar_painel", description="[ADMIN] Cria a mensagem fixa do placar que atualiza sozinha.")
    ...
    async def criar_painel(self, interaction: discord.Interaction, tipo: app_commands.Choice[str]):
        await interaction.response.defer(ephemeral=True)

        embed = await self.gerar_embed_xp(interaction.guild) if tipo.value == "xp" else await self.gerar_embed_voz(interaction.guild)
        ...
        c.execute("REPLACE INTO paineis (tipo, canal_id, mensagem_id) VALUES (?, ?, ?)", 
                  (tipo.value, str(interaction.channel.id), str(msg.id)))
        conn.commit()
        conn.close()

        # Meu nobre, mensagem de sucesso braba aqui
        await interaction.followup.send(f"✅ Painel de {tipo.name} plantado com sucesso, meu nobre! Agora os xupetas podem ver quem é o mais desocupado. Eu atualizo sozinho.", ephemeral=True)
    ...
    async def loop_paineis(self):
        ...
        for tipo, canal_id, msg_id in paineis:
            ...
            if not canal:
                try: canal = await self.bot.fetch_channel(int(canal_id))
                except: continue

            # Acha a mensagem e edita ela
            try:
                msg = await canal.fetch_message(int(msg_id))
                novo_embed = await self.gerar_embed_xp(canal.guild) if tipo == "xp" else await self.gerar_embed_voz(canal.guild)
                ...
                print(f"⚠️ Vish, algum corno apagou o painel de {tipo} no chat. Cria de novo dps com o comando.")
