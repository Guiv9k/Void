...
class Stats(commands.Cog):
    ...
    def preparar_banco(self):
        ...
        cursor = conn.cursor()
        cursor.execute(
            "CREATE TABLE IF NOT EXISTS stats (user_id TEXT, palavra TEXT, data TEXT)"
        )
        conn.commit()
        conn.close()
    ...
    @app_commands.command(
        name="analise",
        description="O Vácuo puxa a capivara e faz o exame toxicológico do chat.",
    )
    async def analise(self, interaction: discord.Interaction):
        ...
        cursor.execute(
            "SELECT palavra, COUNT(palavra) as qtd FROM stats GROUP BY palavra ORDER BY qtd DESC LIMIT 5"
        )
        ...
        if not comuns or total_palavras == 0:
            await interaction.followup.send(
                "🌑 O vácuo tá limpo. Ninguém falou merda suficiente ainda.",
                ephemeral=True,
            )
            conn.close()
            return
        ...
        palavras_texto = "\n".join(
            [f"> 🗣️ **{p.upper()}**: falada `{c}` vezes" for p, c in comuns]
        )
        ...
        embed.add_field(
            name="💩 As merdas mais ecoadas aqui:", value=palavras_texto, inline=False
        )
        embed.add_field(
            name="📊 Laudo Psiquiátrico",
            value=f"> 🌪️ **Nível de Shitpost/Caos:** `{caos}%`\n> 🧠 **QI Sobrevivente:** `{intelecto}%`",
            inline=False,
        )
        embed.set_thumbnail(
            url=(
