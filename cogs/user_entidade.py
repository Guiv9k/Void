...
class Entidade(commands.Cog):
    ...
    async def on_message(self, message):
        ...
        if "void" in conteudo and "?" in conteudo:
            respostas_void = [
                "Ai dento, me deixa em paz.",
                "Tá me chamando por quê, xupeta?",
                "Aham q lindo me paga um boquete dps de me marcar.",
            ]
        ...
        if 2 <= hora <= 5:
            if random.random() < 0.10:
                await message.reply(
                    f"⚠️ **ALERTA DE DESOCUPADO:** {message.author.mention}, vai dormir xupeta! Madrugada é pra quem trampa, liso do caralho."
                )

    # --- SISTEMA DE PÉROLAS (Antigo) ---
    @app_commands.command(
        name="perola",
        description="Eterniza uma pérola (merda) que alguém falou no servidor.",
    )
    @app_commands.describe(
        membro="Quem soltou a pérola?", frase="A merda que o xupeta falou"
    )
    async def perola(
        self, interaction: discord.Interaction, membro: discord.Member, frase: str
    ):
        if len(frase) > 500:
            await interaction.response.send_message(
                "❌ Ai dento! Tu quer eternizar a Bíblia? Manda uma frase mais curta, mísera.",
                ephemeral=True,
            )
            return
        ...
        cursor.execute(
            "INSERT INTO quotes VALUES (?, ?, ?)", (str(membro.id), frase, data_hoje)
        )
    ...
    @app_commands.command(
        name="flashback", description="Traz uma pérola alheia do passado."
    )
    async def flashback(self, interaction: discord.Interaction):
        ...
        if res:
            ...
            await interaction.response.send_message(
                "🌑 O vácuo ainda não tem pérolas salvas. Usem o `/perola` primeiro, bando de liso."
            )
