...
class Maldicao(commands.Cog):
    ...
    @app_commands.command(
        name="amaldicoar",
        description="Pague 1000 conto pro agiota infernizar a vida de alguém por 5 min.",
    )
    async def amaldicoar(
        self, interaction: discord.Interaction, vitima: discord.Member
    ):
        if vitima.id == interaction.user.id or vitima.bot:
            await interaction.response.send_message(
                "❌ Tá querendo jogar praga em si mesmo ou no bot, xupeta? Ai dento, escolhe um alvo de verdade.",
                ephemeral=True,
            )
            return
        ...
        if not res or res[0] < PRECO_MALDICAO:
            conn.close()
            await interaction.response.send_message(
                f"💀 Tu é liso demais pra isso. Custa `{PRECO_MALDICAO}` moedas pra jogar praga em alguém. Vai trampar, CLT!",
                ephemeral=True,
            )
            return
        ...
        c.execute(
            ...
            (PRECO_MALDICAO, str(interaction.user.id)),
        )
        conn.commit()
        conn.close()
