...
class Perfil(commands.Cog):
    ...
    @app_commands.command(
        name="perfil",
        description="Puxa a capivara completa de alguém no banco do Agiota.",
    )
    async def perfil(
        ...
        c.execute(
            "SELECT moedas, nivel, xp FROM usuarios WHERE user_id = ?", (alvo_id,)
        )
        stats = c.fetchone()
        ...
        try:
            c.execute(
                "SELECT categoria, quantidade FROM reputacao WHERE user_id = ?",
                (alvo_id,),
            )
            reps = c.fetchall()
            for rep in reps:
        ...
        try:
            c.execute(
                "SELECT parceiro_id FROM casamentos WHERE user_id = ?", (alvo_id,)
            )
        ...
        if (
            dict_reps["caos"] > dict_reps["genio"]
            and dict_reps["caos"] > dict_reps["bondade"]
        ):
        ...
        # Linha 1: Status Base
        embed.add_field(
            name="💰 Conta do Agiota", value=f"`{moedas:,}` moedas", inline=True
        )
        embed.add_field(
            name="⭐ Moral no Server", value=f"`{total_rep}` REP", inline=True
        )
        embed.add_field(
            name="🏆 Estante", value=f"`{total_conquistas}` troféus", inline=True
        )
        ...
        )  # Campo invisível pra alinhar bonitinho
        ...
        # Rodapé Oficial
        embed.set_footer(
            text=f"ID do Liso: {alvo.id} • CPF negativado",
