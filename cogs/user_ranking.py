...
class RankingPaginacao(discord.ui.View):
    ...
    def criar_embed(self):
        ...
        embed.add_field(
            ...
            value=texto if texto else "Nenhuma alma encontrada.",
            inline=False,
        )
        embed.set_footer(
            text=f"Página {self.pagina_atual + 1} de {self.total_paginas} • Vai caçar o que fazer, mísera."
        )
        return embed

    @discord.ui.button(
        label="◀ Voltar", style=discord.ButtonStyle.secondary, custom_id="prev"
    )
    ...
    @discord.ui.button(
        label="Avançar ▶", style=discord.ButtonStyle.primary, custom_id="next"
    )
...
class Ranking(commands.Cog):
    ...
    def gerar_card_ranking(self, guilda, top_5_data, bot_avatar_url):
        ...
        draw.text(
            (x_titulo + 5, y_titulo + 5),
            texto_titulo,
            font=f_titulo,
            fill=(0, 255, 255, 100),
        )
        draw.text(
            (x_titulo, y_titulo), texto_titulo, font=f_titulo, fill=(255, 255, 255, 255)
        )
        ...
        draw.line(
            [(x_barrinha, y_barrinha), (x_barrinha + largura_barrinha, y_barrinha)],
            fill=(0, 255, 255, 150),
            width=5,
        )
        ...
        for i, user_data in enumerate(top_5_data):
            ...
            draw.text(
                (150, y_linha + 45), f"#{rank}", font=f_nick, fill=(0, 255, 255, 255)
            )
            ...
            try:
                ...
                draw.ellipse(
                    (280, y_linha + 15, 280 + 100, y_linha + 15 + 100),
            ...
            draw.text(
                (420 + 2, y_linha + 25 + 2), user_nick, font=f_nick, fill=(0, 0, 0, 255)
            )
            draw.text(
                (420, y_linha + 25), user_nick, font=f_nick, fill=(255, 255, 255, 255)
            )
    ...
    @app_commands.command(
        name="xp_ranking",
        description="O vácuo expõe a cara dos 5 maiores desocupados do servidor em uma imagem.",
    )
    async def xp_ranking(self, interaction: discord.Interaction):
        ...
        c = conn.cursor()
        c.execute(
            "SELECT user_id, xp, nivel FROM usuarios ORDER BY nivel DESC, xp DESC LIMIT 5"
        )
        top_5 = c.fetchall()
        conn.close()
        ...
        if not top_5:
            await interaction.followup.send(
                "🌀 O vácuo ainda não consumiu ninguém. Bando de CLT ocupado.",
                ephemeral=True,
            )
            return
        ...
        await interaction.followup.send(
            content="📸 Tá aí a foto dos maiores frangos do servidor:",
            file=discord.File(fp=img_buffer, filename="ranking.png"),
        )

    @app_commands.command(
        name="ranking_all",
        description="Lista completa pra você ver o quão liso você está no ranking geral.",
    )
    async def ranking_all(self, interaction: discord.Interaction):
        ...
        c = conn.cursor()
        c.execute(
            "SELECT user_id, xp, nivel FROM usuarios ORDER BY nivel DESC, xp DESC"
        )
        ...
        if not todos_usuarios:
            await interaction.followup.send(
                "🌀 O vácuo está vazio. Ninguém pisou aqui ainda.", ephemeral=True
            )
            return

        view = RankingPaginacao(
            todos_usuarios, interaction.guild, self.bot.user.display_avatar.url
        )
