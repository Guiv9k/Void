...
class Reputacao(commands.Cog):
    ...
    def criar_tabelas(self):
        ...
        c = conn.cursor()
        c.execute('''CREATE TABLE IF NOT EXISTS reputacao 
                     (user_id TEXT, categoria TEXT, quantidade INTEGER, UNIQUE(user_id, categoria))''')
        conn.commit()
        conn.close()

    @app_commands.command(name="rep", description="Julgue um xupeta do servidor dando um ponto de reputação pra ele.")
    @app_commands.describe(membro="Quem você vai julgar?", categoria="Qual a fama dessa pessoa?")
    ...
    async def dar_rep(self, interaction: discord.Interaction, membro: discord.Member, categoria: app_commands.Choice[str]):
        if membro.id == interaction.user.id or membro.bot:
            await interaction.response.send_message("❌ Querendo mamar a si mesmo ou dar rep pro bot? Ai dento, xupeta! Escolhe um alvo de verdade.", ephemeral=True)
            return
        ...
        if user_id in self.cooldowns and tempo_atual - self.cooldowns[user_id] < 7200:
            ...
            if horas > 0:
                await interaction.response.send_message(f"⏳ Segura a emoção, fiscal de cuheio! Tu só pode julgar alguém de novo em **{horas}h e {minutos}m**.", ephemeral=True)
            else:
                await interaction.response.send_message(f"⏳ Tá emocionado? Tu só pode julgar alguém de novo em **{minutos} minutos**.", ephemeral=True)
            return
        ...
        c.execute('''INSERT INTO reputacao (user_id, categoria, quantidade) 
                     VALUES (?, ?, 1) 
                     ON CONFLICT(user_id, categoria) 
                     DO UPDATE SET quantidade = quantidade + 1''', (str(membro.id), categoria.value))
        conn.commit()
        conn.close()
        ...
        embed.set_footer(text="Aham q lindo, me paga um boquete dps desse julgamento.", icon_url=self.bot.user.display_avatar.url)
